(function($) {
    ...
    $(function(){
        $('.wrapperMain form').on('submit', function(event) {
            $.ajax({
                url: "https://www.googleapis.com/customsearch/v1?q="+encodeURIComponent(this.elements[0].value)+"&key=AIzaSyBv9Q39cscpfUGrrUueGU7Pf1xOELB3qEU&cx=000283222127530457480:fcqttmqm5o0&callback=?",
                dataType: "jsonp",
                success: function (data) {
                    if (data === null) {
                        $('#results').html("Error");
                    } else {
                        //отключим враппер ввода с лого
                        var wrapper = document.querySelector('.wrapperMain'); 
                        wrapper.style.display='none';
              
                        // отправим полученные данные в шаблонизатор                     
                        var template = $('#resultsFull').html();
